...
| 70 | **native 值模型断层（#L-22）→ $host.make 构造通道 + S-18 预警**（v0.2.56 第九轮）：native 块返回的 plain object / plain array 不带运行时 `__struct`/`__enum` 标记 → **foreign 值**（字段直通可用，`.clone()`/方法派发/match 模式匹配全失效 —— Curator 实录 `entities.clone()` 运行期 panic「foreign 没有方法 clone」，此前唯一出路是「native 拍平字符串 + HSL 侧 split_once 逐字段重建」的 12 行定式）。**修复**：(a) `$host.make` 官方构造通道 —— `$host.make("Entity", { kind, value, confidence })` 产出带标记的合法结构体（字段完备性/多余性校验与结构体字面量同规则）；`$host.make("Status::Pair", { a, b })` 构造命名字段变体；元组变体走数组 payload（`"Carrier::Boxed", [8, 9]`）；单元变体无 payload；prelude 族（`Result::Ok/Err`、`Option::Some/None`）显式镜像。实现位于 interp.linkProgram（结构体/枚举注册表就绪后幂等注入 hostApi）；字段校验失败可观测报错（缺字段/未知类型/多余字段/元组长度）。(b) **S-18 静态预警**（dhv-ts checker + dhv typecheck 双端同口径）：`let <含 struct/enum 族名的注解> = native <...>` 且 native 体无 `$host.make` → 警告「产物为 foreign 值」（只判同现场 let 注解 + 初始化器，零误报面窄；fn 返回值经变量中转不判 —— S-14 v3 的追踪不覆盖运行期值标记）。**conformance 第 6 段锁定预警对等**（退出码对拍看不见警告是否产出 —— warn fixture 双端必须告警、legal fixture 双端必须零告警）| native python 块无 $host 注入（JSON 编组进出，构造通道仅 typescript/javascript 后端可用）；emit 投射不翻译 $host.make（contract 后端围栏语义） |
