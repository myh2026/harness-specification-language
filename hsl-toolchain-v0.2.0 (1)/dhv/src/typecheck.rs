...
pub struct TypeChecker {
    ...
    /// enum 注册表：enum 名 → 变体名列表（S6 穷尽性校验）
            ...
            if let Some(span) = wildcard_span {
                self.diags.push(
                    Diagnostic::error(
                        ...
                        "graph AgentLoop 内对 Action 的 match 不允许 `_` 通配兜底".to_string(),
                        span,
                    )
                    .note("S6 强制显式列出所有变体：新增 Action 变体时编译器逼你直面新分支"),
                );
            }
        }
